#!/usr/bin/env python3
"""Test runner for CF_SHA256 — uses cocotb.runner API."""
...
IP_NAME = "CF_SHA256"
YAML_FILE = str(PROJ_PATH / "../../CF_SHA256.yaml")

BUSES = ["APB", "AHB", "WISHBONE"]
BUS_CONFIGS = {
    "APB": {
        "design_name": "CF_SHA256_APB",
        "clk": "PCLK",
        "rst": "PRESETn",
        "macro": "BUS_TYPE_APB",
    },
    "AHB": {
        "design_name": "CF_SHA256_AHBL",
        "clk": "HCLK",
        "rst": "HRESETn",
        "macro": "BUS_TYPE_AHB",
    },
    "WISHBONE": {
        "design_name": "CF_SHA256_WB",
        "clk": "clk_i",
        "rst": "rst_i",
...
TESTS = [
    "WriteReadRegsTest",
    "SHA256SingleBlockTest",
    "SHA224SingleBlockTest",
    "SHA256MultiBlockTest",
    "InterruptTest",
    "CoverageClosureTest",
]
...
def get_rtl_sources():
    ...
    for name in sha_rtl_files:
        f = rtl_dir / name
        if f.exists():
            sources.append(str(f))

    wrapper_dir = rtl_dir / "bus_wrappers"
    for name in ("CF_SHA256_APB.v", "CF_SHA256_AHBL.v", "CF_SHA256_WB.v"):
...
def _fallback_reports(out_dir, results_list):
    ...
    md_lines.append("")
    md_lines.append("# CF_SHA256 Verification Results")
    md_lines.append("")
