#!/usr/bin/env python3
"""Test runner for CF_AES — uses cocotb.runner API."""
...
IP_NAME = "CF_AES"
YAML_FILE = str(PROJ_PATH / "../../CF_AES.yaml")

BUSES = ["APB", "AHB", "WISHBONE"]
BUS_CONFIGS = {
    "APB": {
        "design_name": "CF_AES_APB",
        "clk": "PCLK",
        "rst": "PRESETn",
        "macro": "BUS_TYPE_APB",
    },
    "AHB": {
        "design_name": "CF_AES_AHBL",
        "clk": "HCLK",
        "rst": "HRESETn",
        "macro": "BUS_TYPE_AHB",
    },
    "WISHBONE": {
        "design_name": "CF_AES_WB",
        "clk": "clk_i",
        "rst": "rst_i",
...
def get_rtl_sources():
    ...
    for name in aes_rtl_files:
        f = rtl_dir / name
        if f.exists():
            sources.append(str(f))

    wrapper_dir = rtl_dir / "bus_wrappers"
    for name in ("CF_AES_APB.v", "CF_AES_AHBL.v", "CF_AES_WB.v"):
...
def _fallback_reports(out_dir, results_list):
    ...
    md_lines.append("")
    md_lines.append("# CF_AES Verification Results")
    md_lines.append("")
