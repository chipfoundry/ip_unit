#!/usr/bin/env python3
"""Test runner for CF_GPIO8 — uses cocotb.runner API."""
...
IP_NAME = "CF_GPIO8"
YAML_FILE = str(PROJ_PATH / "../../CF_GPIO8.yaml")

BUSES = ["APB", "AHB", "WISHBONE"]
BUS_CONFIGS = {
    "APB": {
        "design_name": "CF_GPIO8_APB",
        "clk": "PCLK",
        "rst": "PRESETn",
        "macro": "BUS_TYPE_APB",
    },
    "AHB": {
        "design_name": "CF_GPIO8_AHBL",
        "clk": "HCLK",
        "rst": "HRESETn",
        "macro": "BUS_TYPE_AHB",
    },
    "WISHBONE": {
        "design_name": "CF_GPIO8_WB",
        "clk": "clk_i",
        "rst": "rst_i",
...
def get_rtl_sources():
    ...
    sources.append(str(rtl_dir / "CF_GPIO8.v"))

    wrapper_dir = rtl_dir / "bus_wrappers"
    for name in ("CF_GPIO8_APB.v", "CF_GPIO8_AHBL.v", "CF_GPIO8_WB.v"):
...
def _fallback_reports(out_dir, results_list):
    ...
    md_lines.append("")
    md_lines.append("# CF_GPIO8 Verification Results")
    md_lines.append("")
    md_lines.append(f"**Tests:** {passed}/{total} passed, {failed} failed")
    md_lines.append("")

    buses_in = sorted(set(r["bus"] for r in results_list))
    ...
    md_lines.append("")
    md_lines.append("| Test |" + "|".join(f" {b} " for b in buses_in) + "|")
    md_lines.append("|------|" + "|".join("---:" for _ in buses_in) + "|")
