#!/usr/bin/env python3
"""Test runner for CF_PSRAM_CTRL — uses cocotb.runner API."""
...
IP_NAME = "CF_PSRAM_CTRL"
YAML_FILE = str(PROJ_PATH / "../../CF_PSRAM_CTRL.yaml")

BUSES = ["AHB"]
BUS_CONFIGS = {
    "AHB": {
        "design_name": "CF_PSRAM_CTRL_AHBL",
        "clk": "HCLK",
        "rst": "HRESETn",
        "macro": "BUS_TYPE_AHB",
    },
...
def get_rtl_sources():
    ...
    sources.append(str(rtl_dir / "CF_PSRAM_CTRL.v"))

    wrapper_dir = rtl_dir / "bus_wrapper"
    wrapper = wrapper_dir / "CF_PSRAM_CTRL_AHBL.v"
    if wrapper.exists():
        sources.append(str(wrapper))

    sram_model = PROJ_PATH / "../models/23LC1024.v"
    if sram_model.exists():
        sources.append(str(sram_model))
    else:
        sram_model_legacy = PROJ_PATH / "../uvm-python/23LC1024.v"
        if sram_model_legacy.exists():
            sources.append(str(sram_model_legacy))
...
def _fallback_reports(out_dir, results_list):
    ...
    md_lines.append("")
    md_lines.append(f"# {IP_NAME} Verification Results")
    md_lines.append("")
    md_lines.append(f"**Tests:** {passed}/{total} passed, {failed} failed")
    md_lines.append("")

    buses_seen = sorted(set(r["bus"] for r in results_list))
    ...
    md_lines.append("")
    md_lines.append("| Test |" + "|".join(f" {b} " for b in buses_seen) + "|")
    md_lines.append("|------|" + "|".join("---:" for _ in buses_seen) + "|")
    for test in test_names:
        row = f"| {test} |"
        for bus in buses_seen:
