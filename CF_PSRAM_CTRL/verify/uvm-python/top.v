...
module top();
    ...
    `ifdef BUS_TYPE_AHB
        ...
        // for example
        CF_PSRAM_CTRL_AHBL dut(.HCLK(CLK), .HRESETn(RESETn), .HADDR(HADDR), .HWRITE(HWRITE), .HSEL(HSEL), .HSIZE(HSIZE), .HTRANS(HTRANS), .HWDATA(HWDATA), .HRDATA(HRDATA), .HREADY(HREADY),.HREADYOUT(HREADYOUT), .sck(sck), .ce_n(ce_n), .din(din), .dout(dout), .douten(douten));
    `endif // BUS_TYPE_AHB
    M23LC1024 slave(
        .SCK(sck),
        .CS_N(ce_n),
