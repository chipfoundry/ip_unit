#!/usr/bin/env python3
"""Test runner for CF_USB_CDC_WRAPPER — uses cocotb.runner API."""
...
IP_NAME = "CF_USB_CDC_WRAPPER"
YAML_FILE = str(PROJ_PATH / "../../CF_USB_CDC_WRAPPER.yaml")

BUSES = ["APB", "AHB"]
BUS_CONFIGS = {
    "APB": {
        "design_name": "CF_USB_CDC_WRAPPER_APB",
        "clk": "PCLK",
        "rst": "PRESETn",
        "macro": "BUS_TYPE_APB",
    },
    "AHB": {
        "design_name": "CF_USB_CDC_WRAPPER_AHBL",
        "clk": "HCLK",
        "rst": "HRESETn",
        "macro": "BUS_TYPE_AHB",
    },
...
def get_rtl_sources():
    ...
    usb_cdc_rtl = ip_dir / "usb_cdc" / "hdl" / "rtl"
    if usb_cdc_rtl.exists():
        for vf in sorted(usb_cdc_rtl.glob("*.v")):
            sources.append(str(vf))

    sources.append(str(rtl_dir / "CF_USB_CDC_WRAPPER.v"))

    wrapper_dir = rtl_dir / "bus_wrappers"
    for name in ("CF_USB_CDC_WRAPPER_APB.v", "CF_USB_CDC_WRAPPER_AHBL.v"):
...
def _fallback_reports(out_dir, results_list):
    ...
    md_lines.append("")
    md_lines.append("# CF_USB_CDC_WRAPPER Verification Results")
    md_lines.append("")
